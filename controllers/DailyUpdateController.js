    ...
    if(user.email == admin){
        if(cases !== null && cases !== undefined){
            if(deaths !== null && deaths !== undefined){
                ...
                else return res.status(400).json({ message: 'recoveries must haven to create' });
            }
            else return res.status(400).json({ message: 'deaths must haven to create' });
        }
        else return res.status(400).json({ message: 'cases must haven to create' });
    }
...
exports.update = (req,res)=>{
    ...
    if(user.email == admin){
        Daily.updateOne({_id:id},{$set:req.body}).then(docs=>{
            if(docs){
                Daily.findById(id).then(docs=>{
                    return res.json({ message: 'update successfully!',DailyUpdate:docs });
                }).catch(err=>{
                    return res.status(400).json(err);
                })
            }
